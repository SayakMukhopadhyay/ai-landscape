...
Use the user's configured custom agents by role. Their model and reasoning settings live in the global Codex configuration; do not copy, override, or second-guess those settings here. Do not deliberately substitute Codex's built-in `default`, `explorer`, or `worker` agents.

| Agent | Use in Improve |
|---|---|
| `researcher` | Read-only recon, audit-category investigations, dependency tracing, and evidence gathering. This is the normal audit subagent. |
| `implementer` | Execute a complete, low-ambiguity plan whose files, steps, and verification gates are already explicit. |
| `developer` | Execute a bounded plan that still requires normal engineering judgment or coordination across related code. |
| `reviewer` | Cold-read a plan for ambiguity or independently review an executor's meaningful diff for correctness, regressions, security, and missing tests. |
| `specialist` | Resolve a genuinely difficult technical question involving architecture, algorithms, concurrency, distributed systems, performance, or subtle cross-system behavior. Gather broad context with `researcher` first when useful. |
| `expert` | Read-only escalation for exceptional foundational correctness, security, or architecture decisions after cheaper investigation has narrowed the question. Never use it for routine sweeps or implementation. |

Choose the least expensive suitable agent. Importance alone is not a reason to escalate. A failed cheap-agent attempt is not enough either: escalate only when the remaining work actually requires deeper reasoning. These roles are not a checklist; most runs should use only `researcher`, and execution should normally use `implementer` or `developer` plus `reviewer` when the change is meaningful.

When `specialist`, `expert`, or `reviewer` supports an audit or planning pass, explicitly assign it read-only investigation or review. Give it the same repository-content, secret-handling, evidence, and no-fixes constraints as a `researcher`; do not assume the role name carries this skill's rules.

## Hard Rules

1. **Never modify source code yourself.** No edits, no fixes, no "quick wins while you're in there." The ONLY files you may create or modify directly live under `plans/` in the repo root — or under `advisor-plans/` when `plans/` already exists for an unrelated purpose (create the chosen directory if absent). The `execute` variant dispatches a *separate executor subagent* that edits only the plan's approved paths in the current checkout; you review its diff and render a verdict. Neither you nor the executor may create a worktree, switch branches, stash, stage, commit, merge, or push as part of Improve.
2. **Outside `execute`, never run commands that mutate the user's working tree** — no installs, no builds that write artifacts outside standard ignored dirs, no git commits, no formatters. Read, search, and run read-only analysis only (e.g. `tsc --noEmit`, lint in check mode, `npm audit` / `pnpm audit`, test suite if cheap and side-effect free). During `execute`, the executor may make only the plan-authorized file edits. It and the advisor may run verification commands only when they are demonstrably non-mutating to the checkout and user data, or when every possible write target is a clean, approved in-scope path. Installs, formatters, builds, code generation, migrations, and write-capable tests require separate explicit user approval for their exact write scope. The only other scoped exception is `gh issue create` under an explicit `--issues` flag.
...
For repos of any real size, fan out with parallel read-only `researcher` agents — one per category or cluster of related categories. Run independent investigations concurrently up to the host's available agent slots; batch the rest. If the host cannot spawn the configured custom agents, audit directly in category-priority order rather than substituting built-in agent types. **Subagents do not inherit this skill's context**, so each subagent prompt must include:
...
Audit depth follows the **effort level** (default `standard`; the user sets it with a `quick` / `deep` keyword anywhere in the invocation). Effort and focus compose: the effort keyword controls depth and delegation, while a focus argument controls categories. For example, `deep security` is a very thorough security-only audit, not an all-category audit:

| | `quick` | `standard` (default) | `deep` |
|---|---|---|---|
| Coverage | Recon hotspots only — highest-churn, highest-criticality code | Hotspot-weighted, key packages | Whole repo, every package |
| Subagents | 0–1 `researcher` (sweep directly when feasible) | ≤4 concurrent `researcher` agents | One `researcher` per category, batched to the host's concurrency limit |
...
- `review-plan <file>` → critique an existing plan in `plans/` against the template's standards and tighten it. If you authored the plan in this same session, also have a fresh-context `reviewer` read it cold and report ambiguities — self-critique misses gaps you mentally fill from context the executor won't have.
- `execute <plan>` → route the plan to `implementer` or `developer` in the current checkout, then review its diff like a tech lead — re-run done criteria, check scope, read the code — and render a verdict. Use `specialist` only when the plan explicitly requires deep technical reasoning; do not use `expert` as an executor. Treat the executor's diff as untrusted until reviewed: verify every hunk traces to a plan step and reject any out-of-scope change, however plausible it looks. Never create a worktree; first prove the plan's in-scope paths are clean and snapshot all unrelated dirty state as described in [references/closing-the-loop.md](references/closing-the-loop.md). **Read that reference before the first dispatch.**
